...
func GetAllRoutes(isdeleted, isresp, isblocked bool, price int, uid uint) (route []models.Route, err error) {
	err = db.GetconnectDB().Joins("Join users ON users.id=routes.client_id OR users.id=routes.driver_id").Where("routes.is_response=? AND routes.is_deleted=? AND routes.price=?", isresp, isdeleted, price).Where("routes.client_id=? OR routes.driver_id=?", uid).Where("users.is_blocked=? AND users.is_deleted=?", isblocked, isdeleted).Order("routes.id").Find(&route).Error
	if err != nil {
...
func GetAllRoutesByID(isdeleted, isblocked bool, uid, rid uint) (route []models.Route, err error) {
	err = db.GetconnectDB().Joins("Join users ON users.id=routes.client_id OR users.id=routes.driver_id ").Where("routes.is_deleted=?", isdeleted).Where("routes.id=?  OR (routes.client_id=? OR routes.driver_id=?)", rid, uid).Where("users.is_blocked=? AND users.is_deleted=?", isblocked, isdeleted).Order("routes.id").Find(&route).Error
	if err != nil {
