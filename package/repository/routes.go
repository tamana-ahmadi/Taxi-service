...
func GetAllRoutes(isresp, isdeleted bool, price int) (route []models.Route, err error) {
	err = db.GetconnectDB().Raw("Select r.from, r.into, r.is_response, COUNT(DISTINCT r.distance), COUNT(DISTINCT r.pricekm), SUM(r.all_price), COUNT(DISTINCT CASE WHEN u.role='user' THEN u.id END) as client_id, COUNT(DISTINCT CASE WHEN u.role='driver' THEN u.id END) as driver_id FROM routes r, users u Where r.client_id=u.id OR r.driver_id=u.id AND r.is_response=? AND r.is_deleted=? AND all_price<=? GROUP BY r.from,r.into,r.is_response ORDER BY r.all_price DESC", isresp, isdeleted, price).Scan(&route).Error
	if err != nil {
