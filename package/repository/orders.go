...
func OrdersReport(isdone, isdeletedo, isdeletedt, isdeletedr, isresp, isblocked, isdeletedu bool) ([]models.Order, error) {
	var order []models.Order
	err := db.GetconnectDB().Select("t.comp_title, o.is_done, COUNT(DISTINCT CASE WHEN u.role='user' THEN u.id END) AS clientid, COUNT(DISTINCT CASE WHEN u.role='driver' THEN u.id END) AS driverid, COUNT(DISTINCT o.route_id) AS routeid, SUM(r.distance) as distance, SUM(r.price) as price").Joins("LEFT JOIN taxicompanies t ON t.id = o.taxicomp_id").Joins("LEFT JOIN routes r ON r.id=o.route_id").Joins("LEFT JOIN users u ON u.id = o.client_id OR u.id = o.driver_id").Where("o.is_done = ? AND o.is_deleted = ? AND t.is_deleted = ?", isdone, isdeletedo, isdeletedt).Where("r.is_deleted=? AND r.is_response=?", isdeletedr, isresp).Where("u.is_blocked = ? AND u.is_deleted = ?", isblocked, isdeletedu).Group("t.comp_title, o.is_done").Order("clientid, driverid DESC").Find(&order).Error
	if err != nil {
