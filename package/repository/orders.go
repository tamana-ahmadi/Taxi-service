...
func OrdersReport(isdone, isdeletedo, isdeletedt, isdeletedr, isresp, isblocked, isdeletedu bool) ([]models.Order, error) {
	var order []models.Order
	err := db.GetconnectDB().Select("taxicompanies.comp_title, orders.is_done, COUNT(DISTINCT CASE WHEN users.role='user' THEN users.id END) AS clientid, COUNT(DISTINCT CASE WHEN users.role='driver' THEN users.id END) AS driverid, COUNT(DISTINCT orders.route_id) AS routeid, SUM(routes.distance) as distance, SUM(routes.price) as price").Preload("User").Preload("Route").Preload("Taxicomp").Joins("LEFT JOIN taxicompanies  ON taxicompanies.id = orders.taxicomp_id").Joins("LEFT JOIN routes  ON routes.id=orders.route_id").Joins("LEFT JOIN users  ON users.id = orders.client_id OR users.id = orders.driver_id").Where("orders.is_done = ? AND orders.is_deleted = ? AND taxicompanies.is_deleted = ?", isdone, isdeletedo, isdeletedt).Where("routes.is_deleted=? AND routes.is_response=?", isdeletedr, isresp).Where("users.is_blocked = ? AND users.is_deleted = ?", isblocked, isdeletedu).Group("taxicompanies.comp_title, orders.is_done").Order("clientid, driverid DESC").Find(&order).Error
	if err != nil {
