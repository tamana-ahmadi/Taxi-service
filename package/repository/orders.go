...
func OrdersReport(isdone, isdeletedo, isdeletedt, isdeletedr, isresp, isblocked, isdeletedu bool) ([]models.Ord, error) {
	var order []models.Ord
	err := db.GetconnectDB().Select("t.comp_title,orders.is_done, COUNT(DISTINCT CASE WHEN u.role='user' THEN u.id END) AS clientid, COUNT(DISTINCT CASE WHEN u.role='driver' THEN u.id END) AS driverid, COUNT(DISTINCT orders.route_id) AS routeid, SUM(r.distance) as distance, SUM(r.price) as price").Preload("User").Preload("Taxicomp").Preload("Route").Joins("Join taxicompanies  ON t.id = orders.taxicomp_id").Joins("Join routes r ON r.id=orders.route_id").Joins("Join users u ON u.id = orders.client_id OR u.id = orders.driver_id").Where("orders.is_done = ? AND orders.is_deleted = ? AND t.is_deleted = ?", isdone, isdeletedo, isdeletedt).Where("r.is_deleted=? AND r.is_response=?", isdeletedr, isresp).Where("u.is_blocked = ? AND u.is_deleted = ?", isblocked, isdeletedu).Group("t.comp_title, orders.is_done").Order("clientid, driverid DESC").Find(&order).Error
	if err != nil {
