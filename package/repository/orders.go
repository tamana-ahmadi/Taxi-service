...
func EditOrders(routeid, userid, taxicompid, id int) error {
	err := db.GetconnectDB().Save(&models.Order{ID: id, UserID: userid, RouteID: routeid, TaxicompID: taxicompid}).Error
	if err != nil {
...
func OrdersReport(isdone, isdeleted, isresp, isblocked bool) ([]models.Order, error) {
	var order []models.Order
	ordersreport := `SELECT t.comptitle, o.is_done, COUNT(DISTINCT(SELECT u.id FROM users WHERE u.role="user")) AS count_of_clients, COUNT(DISTINCT(SELECT u.id FROM users WHERE u.role="driver")) AS count_of_drivers COUNT(DISTINCT o.route_id) AS count_of_route, SUM(r.distance) as total_distance, SUM(r.price) as total_price FROM orders o LEFT JOIN taxicompanies t ON t.id = o.taxicomp_id LEFT JOIN routes ON routes.id=o.route_id  LEFT JOIN users u ON u.ID = o.user_id WHERE o.is_done = ? AND o.is_deleted = ? AND t.is_deleted = ? AND r.is_deleted=? AND r.is_response=?  AND u.is_blocked = ? AND u.is_deleted = ? GROUP BY t.comptitle, o.is_done ORDER BY count_of_clients, count_of_drivers DESC`
