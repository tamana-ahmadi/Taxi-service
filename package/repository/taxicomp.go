...
func GetAllTaxicomps(isdeletedt, isblocked, isdeletedu bool) (txcm []models.GetAllTaxicomp, err error) {
	err = db.GetconnectDB().Raw("Select t.comp_title,t.isdeleted,u.full_name FROM taxicompanies t, users u Where t.driver_id=u.id AND t.is_deleted=? AND u.is_blocked=? u.is_deleted=?", isdeletedt, isblocked, isdeletedu).Scan(&txcm).Error
	if err != nil {
