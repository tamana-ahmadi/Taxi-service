...
func GetAllTaxicomps(isdeletedt, isblocked, isdeletedu bool) (txcm []models.TaxiComp, err error) {
	err = db.GetconnectDB().Joins("Join users ON users.id=taxicompanies.driver_id").Where("taxicompanies.is_deleted=?", isdeletedt).Where("users.is_blocked=? AND users.is_deleted=?", isblocked, isdeletedu).Find(&txcm).Error
	if err != nil {
