...
func OrdersReport(isresp, isdeletedr, isblocku, isdeletedu bool) (report []models.Route, err error) {
	...
	AND users.is_blocked=? AND users.is_deleted=? GROUP BY from,into,is_response ORDER BY incomes DESC`, isresp, isdeletedr, isblocku, isdeletedu).Scan(&report).Error
	if err != nil {
